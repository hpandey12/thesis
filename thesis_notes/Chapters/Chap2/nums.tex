...
Star-CCM+ uses the one-fluid formulation to implement the volume-of-fluid method. 
 ...
 the closure relations used to close the system of equations.
...
The High Resolution Interface Capturing (HRIC) scheme is the default volume fraction convection scheme used.
 ...
 are suppressed by artificially smoothening the volume fraction gradient across the interface.~\citelater{starccm+}
...
The implementation of the SST k-\(\omega\) model in Star-CCM+ can be further controlled to
 adjust the realizability. Realizability is the positive semi-definiteness of the 
...
Surface tension, although an interfacial force, is implemented using the 
 ...
 individual phase surface tension contributions \(\vect{f}_{\sigma, \ind{i}}\)~\cite{gadaLargeScaleInterface2017}~\citelater{starccm},
 \begin{equation}
