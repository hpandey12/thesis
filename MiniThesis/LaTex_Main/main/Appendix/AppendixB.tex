...
\chapter{Shell Formulations} % Main appendix title

\label{AppendixB} % Change X to a consecutive letter; for referencing this appendix elsewhere, use \ref{AppendixX}

\section{Curviliniear Coordinates}
    \subsection{Proof: \texorpdfstring{$\boldsymbol{a}_{\alpha;\beta} = (\boldsymbol{n} \otimes \boldsymbol{n}) \boldsymbol{a}_{\alpha, \beta}$}{Cov Derivative}}
    ...
    \begin{equation}\label{appendices:cov_tangent_derivative}
        ...
        \end{split}
    \end{equation}
    \subsection{Computation of Components of \texorpdfstring{$b_{\alpha \beta}$}{Curvature tensor}}
    ...
    \end{equation}
%----------------------------------------------------------------------        
\section{Weak Form Derivation} \label{appendices:weak_form_derivation}
...
%%%---------------------------------------
  % w_{a;b}  
\section{Computation of \texorpdfstring{$w_{\alpha ; \beta}$}{w-a;b}}
...
\section{Discretisation of \texorpdfstring{$w_{\alpha ; \beta}$}{w-a;b}}
...
\section{Linearisation} \label{appendices:curvy_linearisations}
\subsection{Linearisation of \texorpdfstring{$\boldsymbol{a_{\alpha}}$}{a-a}}
...
Substituting the discretised value of $\Delta \boldsymbol{x}$ in \eqref{eq:a_a_lin_7},    
    ...
    \end{equation}
%--------------------------------------------
\subsection{Linearisation of \texorpdfstring{$a_{\alpha \beta}$}{a-ab}} 
...
\end{equation}
%-----------------------------------------------------------------------------------------
\subsection{Linearisation of \texorpdfstring{$a^{\alpha \beta}$}{contravariant metric}}
...
\begin{equation}\label{eq:del_a_ab_5}
    \begin{split}
        ...
        &= \frac{1}{a} e^{\alpha \gamma} e^{\delta \beta} \boldsymbol{a}_{\gamma} \Delta \boldsymbol{a}_{\delta} + \frac{1}{a} e^{\alpha \gamma} e^{\delta \beta} \boldsymbol{a}_{\delta} \Delta \boldsymbol{a}_{\gamma} = \frac{1}{a} e^{\alpha \gamma} e^{\delta \beta} \boldsymbol{a}_{\gamma} \Delta \boldsymbol{a}_{\delta} + \frac{1}{a} e^{\alpha \delta} e^{\gamma \beta} \boldsymbol{a}_{\gamma} \Delta \boldsymbol{a}_{\delta} \\
...
\end{equation}
%-----------------------------------------------------------
\subsection{Linearisation of \texorpdfstring{$\tau^{\alpha \beta}$: Derivation of Compliance Matrix}{Kircchhof stress tensor}} \label{appendices:compliance_matrix_curvy}
\begin{itemize}
\item With the help of equations~\eqref{eq:NH_strain} and~\eqref{eq:CGtrace}, the Kircchhof stress tensor can be written as
...
\begin{equation}\label{eq:dJdaab}
    \begin{split}
    \frac{\partial J}{\partial a_{\alpha \beta}} &= \frac{1}{J_{A}} \frac{\partial J_{a}}{\partial (a_{\alpha \beta})} = \frac{1}{J_{A}} \frac{\partial \left(\sqrt{\det{a_{\alpha \beta}}}\right)}{\partial \left(\det{a_{\alpha \beta}}\right)} \frac{\partial \left(\det{a_{\alpha \beta}}\right)}{\partial a_{\alpha \beta}} \\ &= \frac{1}{J_{A}} \frac{1}{2} \left(\det{a_{\alpha \beta}}\right)^{-\frac{1}{2}} \left(\det{a_{\alpha \beta}}\right) \left( a_{\alpha \beta}^{-1} \right)^{T}\\ &= \frac{1}{2 J_{A}}\, J_{a} \,a^{\alpha \beta} = \frac{J}{2} a^{\alpha \beta}
    \end{split}
\end{equation}
...
\item For another Neo-Hookean material with the Cauchy stress tensor given as, 
...
the components of the tensor can be written as
    ...
    \end{equation}
Thus the linearisation of \eqref{eq:kirch_2} using \eqref{eq:cabgd} is
...
\begin{equation}\label{eq:del_kirch_3}
    \begin{split}
        ...
        c^{\alpha \beta \gamma \delta} &= \frac{\mu}{J^{2}} \left[ 4 a^{\alpha \beta} a^{\delta \gamma} - \frac{1}{a} \left( a^{\alpha \gamma} e^{\delta \beta} + e^{\alpha \delta} e^{\gamma \beta}\right)  \right] \boldsymbol{a}_{\gamma} \cdot \mathbf{N}_{, \delta} \Delta \mathbf{x}_{e}
    \end{split}
\end{equation}
\end{itemize}
...
\subsection{Linearisation of \texorpdfstring{$\mathbf{f}_{\text{int}}^{e}$}{Internal Force Vector}}
Given the internal force vector for an element
\begin{equation}\label{eq:f_int}
    ...
    \end{equation}
the linearisation of \eqref{eq:f_int} with respect to $\mathbf{x}$ can be derived as follows
    \begin{equation}\label{eq:del_f_int}
        ...
        &= \int_{\Omega^{e}} \left[ c^{\alpha \beta \gamma \delta} \mathbf{N}_{, \alpha}^{T} \left( \boldsymbol{a}_{\beta} \otimes \boldsymbol{a}_{\gamma} \right) \mathbf{N}_{, \delta} + \mathbf{N}_{, \alpha}^{T} \tau^{\alpha \beta} \mathbf{N}_{, \beta} \right] \, \mathrm{d} A \Delta \mathbf{x}_{e}.
        \end{split}
    \end{equation}

\subsection{Linearisation of \texorpdfstring{$\mathbf{f}_{\text{ext}}^{e}$}{External Force Vector}}
Given the reduced external force vector based for dead loading 
\begin{equation}
    \mathbf{f}_{\mathrm{ext}}^{e} = \int_{\Omega^{e}} \mathbf{N}^{T} p \boldsymbol{n} \, \mathrm{d} a,
\end{equation}
the linearisation carried out with respect to $\mathbf{x}$ and $p$\footnote{only in the case of hydrostatic loading} can be written as follows
    \begin{equation}\label{eq:del_f_ext_dead_2}
        \begin{split}
            \Delta \mathbf{f}_{\mathrm{ext}}^{e} &= \int_{\Omega^{e}} \Delta \left[\mathbf{N}^{T} p \boldsymbol{n} \, \mathrm{d} a\right]\\
            &= \int_{\Omega^{e}} \mathbf{N}^{T} \Delta \left(p\right) \boldsymbol{n} \,d a + \int_{\Omega^{e}}\mathbf{N}^{T} p \Delta\left(\boldsymbol{n}\,da\right)\\
            &= \int_{\Omega^{e}} \mathbf{N}^{T} \Delta \left(\rho \mathbf{g} \cdot \boldsymbol{x}\right) \boldsymbol{n}\,da + \int_{\Omega^{e}} \mathbf{N}^{T} p \Delta\left(\boldsymbol{n}\,da\right)\\
            &= \int_{\Omega^{e}} \mathbf{N}^{T} \rho \left(\underbrace{\Delta \mathbf{g} \cdot \boldsymbol{x}}_{=0} + \mathbf{g} \cdot \Delta \mathbf{x}_{e}\right) \boldsymbol{n}\,da + \int_{\Omega^{e}} \mathbf{N}^{T} p \Delta\left(\boldsymbol{n}\,da\right)\\
            &= \int_{\Omega^{e}} \rho \mathbf{N}^{T} \left( \boldsymbol{g} \cdot \Delta \mathbf{x}_{e} \right) \boldsymbol{n} \, \mathrm{d} a + \int_{\Omega^{e}} \mathbf{N}^{T} p \Delta \left( \boldsymbol{n} \, \mathrm{d} a \right) \\ &= \int_{\Omega^{e}} \rho \mathbf{N}^{T} \left( \boldsymbol{n} \otimes \boldsymbol{g} \right) \, \mathrm{d} a \Delta \mathbf{x}_{e} + \int_{\Omega^{e}} p \mathbf{N}^{T} \left( \boldsymbol{n} \otimes \boldsymbol{a}^{\alpha} - \boldsymbol{a}^{\alpha} \otimes \boldsymbol{n} \right) \mathbf{N}_{, \alpha} \, \mathrm{d} a \Delta \mathbf{x}_{e}
        \end{split}
    \end{equation}
