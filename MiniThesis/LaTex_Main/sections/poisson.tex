...
\subsection{Introduction: Poisson Problem} \label{sec:poisson_weak_form}
The Poisson equation is the simplest and the most famous elliptic partial differential equation [2.1].
...
A sufficiently smooth function \emph{u} satisfying \eqref{def_poisson} is known as a \emph{classical solution} to the boundary value problem [2.1].
...
\subsubsection{Function Spaces and Well-Posedness of the Weak Formulation}
For the equality in \eqref{weak_formulation_2} to make sense, it is no longer necessary for \emph{u} to be twice differentiable. The classical definition of a derivative
 is stated pointwise - thus requiring the function to be defined at all points. The weak derivative involed in the weak formulation is more general and includes functions that
 functions that are not smooth, to a degree, through the domain. This requires the definition of a function space where it is possible to define a weak derivative.
...
\end{equation}
is the special Sobolev space, which is the set of all functions \emph{f} in $H^{1}(\Omega)$ such that $u = 0$ on the boundary of the domain [2.3].
...
The equation \eqref{weak_formulation_2} is well posed in the sense of Hadamard [2.3] - if the all the first derivatives on the left hand side are in $L_{2}(\mathcal{D})$,
 $\overline{q} \in L_{2}(\mathcal{S}_{q})$, and $s \in L_{2}(\mathcal{D})$, there exists a weak solution, $u \in H^{1}(\mathcal{D})$ that is also a solution to \eqref{def_poisson} [2.2]. 
 According to the Lax \& Milgram theorem, the weak solution obtained from the solution of \eqref{weak_formulation_2} is unique [2.4]. 
%-----------------------------------------------------------------
%- well posedness - https://people.inf.ethz.ch/arbenz/FEM17/pdfs/0-19-852868-X.pdf, https://www.kth.se/social/upload/5261b9c6f276543474835292/main.pdf, https://arxiv.org/pdf/1709.08618.pdf

\subsection{Finite Element Method}
Approximation methods can be broadly divided in two categories.
\begin{itemize}
    \item The given function is approximated using linearly independent functions and parameters. This is a \emph{Non-nodal approximation} and is written as,
    \begin{equation*}
        u(x) = P_{1}(x) a_{1} + P_{2}(x) a_{2} + ... + P_{n}(x) a_{n}
    \end{equation*}
...
Nodal approximation methods are preferred over non-nodal approximations as non-nodal approximations require
 ...
 it can make the fulfilment of boundary conditions difficult and require increased computational power.
 Nodal approximation methods involve piecewise defined shape functions. While it may be difficult to satisfy the
 completeness and continuity criterias involved, the advantage of drastically reduced computation offsets the negative.
The finite element method provides a formalism for generating discrete algorithms for approximating
 the solutions of differential equations using nodal approximations. 
 Formally, according to [2.4], 
\begin{definition}
    ...
    \begin{enumerate}[i]
        ...
        \item $\mathcal{N} = {N_{1}, N_{2},...,N_{k}}$ be a basis for the dual space of $\mathcal{P}$ (the set of nodal variables).
    \end{enumerate}
\end{definition}
%
The $K$ defines the element subdomain being used in the calculation. $\mathcal{P}$ delineates the set of
 the shape functions involved in the approximation, and $\mathcal{N}$ is the set of nodal variables ie the nodes at
...
It is assumed that these conditions ensure the interpolation problem using functions in $\mathcal{P}$ is uniquely solvable.
 For proof, refer to []. The approximation of the variable field that would be the solution to the PDE is the basis of $\mathcal{P}$.
...
The construction of a finite element, thus, involves:
 \begin{itemize}
    ...
    \item computing the nodal basis of P with respect to N.
 \end{itemize}
...
\begin{equation}\label{discretisation_definition}
    \overline{\mathcal{D}} = \bigcup^{n_{el}}_{e=1} \overline{\mathcal{D}}^{e}
\end{equation}
...
With the discretisation defined, the functional spaces for the shape functions and the trial solutions can be defined as,
...
\end{equation}
where $\mathbf{N}^{e}$ denotes the $(d \, \times \, d*n_{en})$ array, $n^{e}_{n}$ denotes the number of element nodes, \emph{d} number of degrees of freedom
...
\begin{gather}\label{K_and_f}
    ...
    \boldsymbol{\mathrm{f}}^{e}_{b} = \int_{\mathcal{D}^{e}} N_b s \,dv.
\end{gather}
\subsection{Isoparametric Elements} \label{sec:isoparametric_theory}
...
Chosen for its efficiency and accuracy, Gaussian quadrature optimises the placement of these points such that the $n_{\text{gp}}$ sampling points can integrate
 exactly a polynomial function of degree $(2n_{\text{gp}}-1)$. Moreover, through the utilisation of the isoparametric concept, the bounds of integration need
 not be changed as the bounds of integration for the reference element are $[-1, 1]$.

Using Gaussian quadrature, the contributions to the system matrices $\boldsymbol{\mathrm{K}}$ and $\boldsymbol{\mathrm{f}}$
 can be written as 
 \begin{gather} \label{quadrature_matrices}
    \boldsymbol{\mathrm{K}}^{e}_{ab} = -\int_{\mathcal{D}^{e}} \nabla N_{a} \cdot \nabla N_{b} \, dv =
...
\subsection{Solutions, Errors, Refinements, Convergence}
