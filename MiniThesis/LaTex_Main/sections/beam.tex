...
\subsection{Continuum Mechanics and Kinematical Relations} \label{sec:continuum_kinematics}

<INSERT CLASSIC CONTINUUM MECHANICS IMAGE OF THE POTATO BUT IN ORTHONORMAL BASIS>
\\
\\
According to continuum mechancis, a body $\mathcal{B}$ is defined as a manifold consisting of finite number of particles.
 This body, $\mathcal{B}$, is also assumed to be a $\sigma$ - finite measure space - 
 ...
 each particle of the body in $\mathbb{E}^{3}$.[3.1][3.2] For simplicity, an orthogonal cartesian basis is assumed in this section as
...
In order to ease the description of motion and deformation, it is convenient to assume a \emph{reference configuration}
 at an arbitrary time, $t = t_{0}$. In this configuration, the body is assumed to be unloaded, undeformed, and unstressed.
 For an arbitrary point on the body, $\mathrm{P}_{0}$, a position vector can be defined with respect to the origin of the basis,
...
At any other time, \emph{t}, the configuration of the body will be referred to as the \emph{current configuration}.
 ...
 both configurations are mapped one-to-one. The position vector of the point $\mathrm{P}_{0}$,
 that has now moved due to external forces to a position P, can be defined as,
...
Two consequences follow the assumption of a cartesian basis system. The dual basis to the unit vectors $\boldsymbol{{E}}_{{i}}$ and $\boldsymbol{{e}}_{{i}}$
 reduces to the original basis, thus making the position of the indices immaterial. And
 $\boldsymbol{{E}}_{{i}} = \boldsymbol{{E}}^{{i}} = \boldsymbol{{e}}_{{i}} = \boldsymbol{{E}}^{{i}}$, thus making the unit
...
The position vector of the point P relative to $\mathrm{P}_{0}$ is called the displacement vector, $\boldsymbol{{u}}$,
\begin{equation} \label{displacement_vector}
    \boldsymbol{{u}} = \boldsymbol{{x}} - \boldsymbol{{X}} = {U}^{{i}} \boldsymbol{{E}}_{{i}} = {u}^{{i}} \boldsymbol{{e}}_{{i}}
\end{equation}
To study the deformation of the body two vectorial line elements $\mathrm{d}\boldsymbol{X}$,
 defined as an infinitesimal vector between two points in the reference configuration, 
 and the vectorial line element $\mathrm{d\boldsymbol{x}}$, defined as the infinitesimal vector between the corresponding two points mapped to the current
 configuration, are used. $\mathrm{d\boldsymbol{X}}$ is mapped to $\mathrm{d\boldsymbol{x}}$ with the help of a second order
 called the \emph{deformation gradient}.
...
The deformation gradient further allows the transformation of differential quantities from the reference to the current configuration.
 A differential area element $\mathbf{da}$ in the current configuration can be written in terms of the area element $\mathbf{dA}$ in the reference configuration
 using Nanson's formula,
 \begin{equation} \label{Nansons}
    \mathbf{d}\boldsymbol{a} = \mathbf{n} \,da = J \boldsymbol{F}^{-T} \boldsymbol{N} \,dA = J \boldsymbol{F}^{-T} \mathbf{d}\boldsymbol{A},
 \end{equation}
 where $\boldsymbol{n}$ and $\boldsymbol{N}$ are the normal vectors of the area elements $\mathbf{d}\boldsymbol{a}$ and $\mathbf{d}\boldsymbol{A}$ respectively, and $J$ is the determinant of the deformation gradient
 ...
 \begin{equation} \label{volume}
    dv = J\,dV.
 \end{equation}
The squares of the line elements $\mathbf{d}\boldsymbol{x}$ and $\mathbf{d}\boldsymbol{X}$ are related with the use of the right and left \emph{Cauchy-Green tensors}, $\boldsymbol{C}$,
\begin{align} \label{cauchy_green_tensors}
    \mathbf{d}\boldsymbol{x} \cdot \mathbf{d}\boldsymbol{x} &= \mathbf{d}\boldsymbol{X} \cdot \boldsymbol{C} \mathbf{d}\boldsymbol{X}, \text{ where, } \boldsymbol{C} := \boldsymbol{F}^{T}\boldsymbol{F}^{T},\\
    \mathbf{d}\boldsymbol{X} \cdot \mathbf{d}\boldsymbol{X} &= \mathbf{d}\boldsymbol{x} \cdot \boldsymbol{b}^{-1} \mathbf{d}\boldsymbol{x}, \text{ where, } \boldsymbol{b} := \boldsymbol{F} \boldsymbol{F}^{T},
\end{align}
which enables us to define the strain measures known as the \emph{Green-Lagrange strain tensor} and the \emph{Almansi strain tensor} respectively
...
\end{align}
\subsection{Stress and Balance Equations}
...
The fundamental relations of continuum mechanics are the balance relations.
 They are stated in the following briefly and without proof. The derivations can be found in [3.2] and [3.3].
\subsubsection{Balance of Mass}
The mass of a system is conserved. This implies that the mass of the body in the reference configuration is equal to the 
...
\begin{equation}\label{lin_momentum_balance}
    ...
    \end{cases}  
\end{equation}
where $\boldsymbol{f}$ is the sum of forces, excluding inertia, acting on a body. $\boldsymbol{\ddot{x}}$ and $\boldsymbol{a} = a^{i} \, \boldsymbol{e}_{i}$
 is the acceleration of a particle with the position vector $\boldsymbol{x}$.
The First Piola-Kirchhoff stress tensor can be used to represent the linear momentum balance in the reference configuration.
...
\end{align}
\subsubsection{Balance of Kinetic Energy}
An extension of \eqref{lin_momentum_balance}, is the balance of kinetic energy which states that the power of external forces
 is equal to the sum of the stress power and the change in kinetic energy of a body.
\begin{align}\label{kin_energy}
    P &= W + \dot{K},\\
    &\text{where the power of external forces,} \,\, P = \int_{\mathcal{B}} \rho \,\boldsymbol{\overline{\mathrm{b}}} \cdot \boldsymbol{\mathrm{v}} \, dv + \int_{\partial \mathcal{B}} \boldsymbol{\mathrm{t}} \cdot \mathrm{\boldsymbol{v}} \, da,\\
    &\text{the stress power,} \,\, W = \int_{\mathcal{B}} \boldsymbol{\sigma} : \boldsymbol{\mathrm{d}} \,dv,\\
    &\text{and the change in kinetic energy, }\,\, \dot{K} = \int_{\mathcal{B}} \frac{1}{2} \rho \mathrm{\boldsymbol{v}} \cdot \mathrm{\boldsymbol{v}} \, dv.
\end{align}
...
The First Law of Thermodynamics states that the rate of change of the total energy of a system is equal to the
 sum of the mechanical power of the external loads and the heat supplied to the system.
\begin{align}\label{first_thermo}
    \dot{E} &= P + Q,\\
    &\text{where the total energy,} \, \, E = \int_{\mathcal{B}} \frac{1}{2} \rho \mathrm{\boldsymbol{v}} \cdot \mathrm{\boldsymbol{v}} \, dv + \int_{\mathcal{B}} \rho u \, dv,\\
    &\text{the power of external loads,} \,\, P = \int_{\mathcal{B}} \rho \,\boldsymbol{\overline{\mathrm{b}}} \cdot \boldsymbol{\mathrm{v}} \, dv + \int_{\partial \mathcal{B}} \boldsymbol{\mathrm{t}} \cdot \mathrm{\boldsymbol{v}} \, da,\\
    &\text{and the heat supplied,} \, \, Q = - \int_{\partial \mathcal{B}} \boldsymbol{\mathrm{q}} \cdot \boldsymbol{\mathrm{n}} \, da + \int_{\mathcal{B}} \rho \, r \,dv
\end{align}
...
\subsection{Constitutive Relations, Material Model}
Constitutive relations are the properties of the material under investigation. They characterise the material response of a body [3.2] and
 are necessary to complete the system of differential equations for the solution of boundary value problems [3.3]. Materials in the real world can exhibit incredibly diverse behaviours making it necessary for the constitutive relations to
 have a degree of approximation involved in their formulation. However, the relations must agree with the observed experimental behaviours. Along with the 
 Second Law of Thermodynamics, there are three general principles that all constitutive relations must satisfy, known as \emph{Noll's axioms}.
...
Hyperelastic materials can be characterised by having zero heat dissipation. Thus the stress power \eqref{stress_power}
 ...
 \end{equation}
Comparing equations \eqref{kin_energy} and \eqref{hyper_stress_power}, according to [3.3],
 \begin{align}\label{S_is_Psi}
...
Neo-Hookean materials are examples of hyperelastic materials. The model can be used to approximate the behaviour of
 rubber like materials fairly accurately for strains upto 20\% [3.4]. Given a Neo-Hookean material model defined as,
...
\end{equation}
Based on \eqref{neo_hookean_S} and \eqref{compliance_definition}, the explicit expression for the fourth order tensor <THAT DOES WHAT> known as
 the compliance matrix $\mathbb{C}$, can be written in the indicial form as follows
...
\subsection{Discretisation} \label{sec:nonlinear_discretisation}
...
After the discretisation of the domain into finite elements, the shape functions can be used to discretise the nodal variables
 ...
 \end{align}
where the summation is carried over the $n_ne$ nodes of the element. 
Using the above given definitions, and the weak form of the momentum balance \eqref{weak_form_beam}, the discretised form of the weak form
 in its internal virtual work contributions can be given as,
...
\subsection{Newton Raphson Method and Linearisation}
...
$D \boldsymbol{f}(\boldsymbol{u})$ can be written using the general $G\hat{a}teaux$ derivative. However, if $D \boldsymbol{f}(\boldsymbol{u})$ is linear
 ...
 \begin{equation} \label{frechet}
    D \boldsymbol{f}(\boldsymbol{u}) := \boldsymbol{f} \prime \left(\boldsymbol{u}\right) \cdot \Delta \boldsymbol{u}, \text{ where, } f'(u)_{ij} = \frac{\partial f_{i}}{\partial u^{j}} (u).
 \end{equation}
In \eqref{frechet}, $\boldsymbol{f} \prime \left(\boldsymbol{u}\right)$ can be called the Jacobian or the tangent matrix. 
